#' Visualize transformations on square and rectangle surfaces in the complex plane
#'
...
quad_surface_transform <- function(h = 1, v = 1, x0 = 0, y0 = 0, x_new = expression(x^2 - y^2),
  ...
  if (!is.numeric(h) | !is.numeric(v) | !is.numeric(x0) | !is.numeric(y0)) {
    ...
    my_quad = data.frame(
      x = stats::runif(8000, -h/2 + 0.005, h/2 - 0.005),
      y = stats::runif(8000, -v/2 + 0.005, v/2 - 0.005)
    ) %>%
    rbind(
      data.frame(
        x = stats::runif(500, -h/2, h/2),
        y = rep(v/2, 500)
      )
    ) %>%
      rbind(
